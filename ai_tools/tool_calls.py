...
async def handle_call_agent_price(arguments, thread_main, tool_outputs, db_entry, action_id):
...
async def handle_call_agent_coder(arguments, thread_main, tool_outputs, db_entry, action_id):
    output = call_agent_coder(**arguments, thread_main=thread_main)
    tool_outputs.append({"tool_call_id": action_id, "output": output})
    db_entry.update({"tool": {'instruction': arguments, "timestamp": int(time.time())}})
    return output

async def handle_create_file(arguments, thread_main, tool_outputs, db_entry, action_id):
    output = create_file(**arguments)
    tool_outputs.append({"tool_call_id": action_id, "output": output})
    await send_message_to_hook_async(user_id=thread_main['u_bot_0_id'], messaged_back=(f"'{output}'"))
    db_entry.update({"tool": {'instruction': arguments, "timestamp": int(time.time())}})
    return output

async def handle_execute_file(arguments, thread_main, tool_outputs, db_entry, action_id):
    output = execute_file(**arguments)
    tool_outputs.append({"tool_call_id": action_id, "output": output})
    db_entry.update({"tool": {'instruction': arguments, "timestamp": int(time.time())}})
    return output

async def handle_move_files(arguments, thread_main, tool_outputs, db_entry, action_id):
    if 'fileMoves' in arguments:
        output = move_files(file_moves=arguments['fileMoves'])
        output_str = '\n'.join(output)
        tool_outputs.append({"tool_call_id": action_id, "output": output_str})
        db_entry.update({"tool": {'instruction': arguments, "timestamp": int(time.time())}})
        return output_str
    else:
