...
def handle_call_agent_price(arguments, thread_main, tool_outputs, db_entry, action_id):
...
def handle_call_agent_coder(arguments, thread_main, tool_outputs, db_entry, action_id):
    output = call_agent_coder(**arguments, thread_main=thread_main)
    tool_outputs.append({"tool_call_id": action_id, "output": output})  # Corrected here
    db_entry.update({"tool": {'instruction': arguments, "timestamp": int(time.time())}})
    return output

def handle_create_file(arguments, thread_main, tool_outputs, db_entry, action_id):
    output = create_file(**arguments)
    tool_outputs.append({"tool_call_id": action_id, "output": output})  # Corrected here
    db_entry.update({"tool": {'instruction': arguments, "timestamp": int(time.time())}})
    return output

def handle_execute_file(arguments, thread_main, tool_outputs, db_entry, action_id):
    output = execute_file(**arguments)
    tool_outputs.append({"tool_call_id": action_id, "output": output})  # Corrected here
    db_entry.update({"tool": {'instruction': arguments, "timestamp": int(time.time())}})
    return output

def handle_move_files(arguments, thread_main, tool_outputs, db_entry, action_id):
